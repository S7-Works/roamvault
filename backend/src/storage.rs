...
impl B2Client {
    ...
    pub async fn signed_download_url(&self, key: &str, valid_secs: u64) -> Result<String> {
        let auth = self.authorize().await?;
        ...
        // B2 signed URL via b2_get_download_authorization
        let resp: serde_json::Value = self
            .http
            .post(format!("{}/b2api/v3/b2_get_download_authorization", auth.api_url))
            .header("Authorization", &auth.authorization_token)
        ...
        let token = resp["authorizationToken"].as_str().unwrap_or_default();
        let url = format!(
            "{}/file/{}/{}?Authorization={}",
            auth.download_url,
